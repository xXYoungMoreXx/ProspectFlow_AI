# TEST_STRATEGY.md — Estratégia de Testes do Hefesto
...
describe("QualifyLeadUseCase", () => {
  ...
  beforeEach(() => {
    ...
    useCase = new QualifyLeadUseCase(
      leadRepoMock,
      mcpMock,
      scoringService,
      eventBusMock,
  ...
  it("deve qualificar lead e salvar quando CNPJ ativo", async () => {
    ...
    mcpMock.consultarCNPJ.mockResolvedValue(
      EnrichmentDataFactory.createActive(),
    );
  ...
  it("deve bloquear lead quando CNPJ suspenso", async () => {
    ...
    leadRepoMock.findById.mockResolvedValue(lead);
    mcpMock.consultarCNPJ.mockResolvedValue(
      EnrichmentDataFactory.createSuspended(),
    );
...
describe("Security: Magic Bytes em Uploads", () => {
  ...
  it("deve rejeitar imagem gerada por IA com magic bytes inválidos", async () => {
    ...
    await expect(validateMagicBytes.validate(fakeAiImage)).rejects.toThrow(
      SecurityError,
    );
  });
...
test("operador aprova HITL via painel e ação é executada", async ({
  page,
  request,
}) => {
  // Criar HITL de teste via API
  const hitl = await request.post("/api/v1/hitl/test", {
    data: testHITLPayload,
  });
