...
O comportamento dos agentes no CrewAI e LiteLLM é governado primordialmente pelos _System Prompts_. Alterações nesses prompts podem ter efeitos colaterais drásticos. Tratar prompts como "configuração de banco de dados" impede auditoria clara e rollback seguro.
...
Adotamos a estratégia **Prompt-as-Code** combinada com **Git-Versioning** para a fundação. Os prompts _baseline_ são arquivos de texto versionados em repositório (Git).
...
- **Negativas**:
  - Operadores do sistema que desejam _fine-tuning_ precisarão utilizar o Editor na UI, o que cria um "fork" do prompt versionado, exigindo lógica para gerenciar prompts "default" vs "customizados" no banco de dados.
