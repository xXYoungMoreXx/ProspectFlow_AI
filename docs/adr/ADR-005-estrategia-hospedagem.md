...
**Arquitetura distribuída com cada componente na plataforma mais adequada ao seu perfil,
priorizando opções gratuitas.**
...
│ Sites dos clientes   │ Vercel / Netlify (free tier, por     │
│                      │ projeto — cada cliente = 1 projeto)  │
├──────────────────────┼──────────────────────────────────────┤
│ API Backend Fastify  │ Railway (free tier: $5 crédito/mês)  │
│ Agent Runtime Python │ Fly.io (free tier: 3 VMs 256MB)      │
├──────────────────────┼──────────────────────────────────────┤
│ Ollama + modelos     │ Máquina local (MVP) →                │
│                      │ Oracle Cloud ARM Free (produção)     │
│ PostgreSQL           │ Supabase free tier OU                │
│                      │ Oracle Cloud Autonomous DB (free)    │
│ Redis                │ Upstash (free: 10k cmd/dia)          │
│ ChromaDB             │ Docker na mesma VM Oracle            │
│ n8n                  │ Docker na mesma VM Oracle            │
│ Evolution API (WPP)  │ Docker na mesma VM Oracle            │
└──────────────────────┴──────────────────────────────────────┘
```

### Oracle Cloud Free Tier — detalhamento (always free, sem expiração)
