...
* **Ameaça:** O Agente Hunter interage externamente se passando por algo que não é, ou sofre *Prompt Injection* para agir contra o escopo definido.
* **Mitigações Implementadas:**
  * *Prompt-as-Code* mantido em readonly no ambiente de execução.
...
* **Mitigações Implementadas:**
  * O Agente Hunter comunica-se com o CRM Core via emissão de *Domain Events* (ex: `LeadQualified`) através da ACL e do barramento assíncrono. O Agente não possui permissão de `UPDATE`/`DELETE` na tabela de Leads diretamente.
...
* **Ameaça:** Durante as atividades de *scraping* / busca, o Agente expõe dados confidenciais nos URLs ou é utilizado via SSRF para scan de redes internas.
* **Mitigações Implementadas:**
...
* **Ameaça:** O *Web Scraping* ou o Loop do Hunter consome todos os tokens da API LLM, gerando exaustão financeira (Wallet Drain / DoS Financeiro).
* **Mitigações Implementadas:**
...
* **Mitigações Implementadas:**
  * Separação estrita de personas e skills no runtime. As skills como `deployer` não estão registradas na *Collection* de Skills injetadas no contexto de LLM do Hunter.
