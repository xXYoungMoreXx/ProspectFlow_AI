# Surgical Analysis: Agentic Architectures for ProspectFlow / AgentePro

## 1. Executive Summary
This document provides a surgical analysis of leading open-source AI agent and MCP (Model Context Protocol) projects. The goal is to extract actionable architectural patterns, integration strategies, and skill definitions that can be legally and technically integrated into the **ProspectFlow/AgentePro** platform, maintaining strict adherence to our Domain-Driven Design (DDD), Hexagonal Architecture, and Security-First (HITL) principles.
...
**Current AgentePro State:** Needs a scalable way to define, version, and load external tools for agents (Hunter, Closer, etc.).
...
*   **Skill Categorization (`goose-skills`)**: Classifying tools into three layers:
    *   *Capabilities*: Atomic, single-purpose tools (e.g., `scrape_cnpj`, `send_telegram_msg`).
    *   *Composites*: Multi-skill chains (e.g., `research_company_and_find_decision_makers`).
...
*   Adopt the Capability/Composite/Playbook hierarchy to organize ProspectFlow's internal sales and prospecting tools.

### 2.2. Agent Persona Definition (Inspired by `agency-agents`)
**Current AgentePro State:** Agents are defined by Persona, LLMConfig, Skills, Rules, etc. (PRD Section 7).
...
**Implementation for AgentePro:**
*   Adopt this `.md` structure for AgentePro's core personas (Hunter, Closer, Builder, QA). Store these in a version-controlled `docs/agents/personas/` directory and build a parser to inject them into the LLM context.
...
**Current AgentePro State:** Requires mandatory Human-in-the-Loop (HITL) for external actions (ADR-004) and supports agent pipelines.

**Extracted Patterns:**
*   **Pipeline with Checkpoints (`opensquad`)**: Agents run in a sequential or DAG pipeline. Crucially, the pipeline contains native *checkpoints* where execution pauses, yielding control back to the host environment (or UI) to request human approval.
...
**Current AgentePro State:** Relies on BullMQ for background task processing.
...
**Current AgentePro State:** Needs localized data for effective prospecting in Brazil.
...
**Implementation for AgentePro:**
*   Deploy `mcp-brasil` as a core infrastructural dependency for the ProspectFlow Agent Runtime.
...
2.  Write integration tests ensuring the ProspectFlow runtime can successfully discover and execute tools from this external MCP server.
...
*   **Data Usage (`mcp-brasil`)**: While the code is MIT, the data sources accessed by `mcp-brasil` have their own terms. AgentePro must adhere to the LGPD (Lei Geral de Proteção de Dados). Specifically, when Hunter agents scrape or aggregate public data (e.g., from Receita Federal or Transparência), the data must only be used for legitimate B2B prospecting purposes and respect opt-out requests.

---
*Analysis completed during architectural audit session. Ready for implementation translation.*
