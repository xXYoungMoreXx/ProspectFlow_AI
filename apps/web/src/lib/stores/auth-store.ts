...
interface AuthState {
  ...
  setAuth: (token: string, refreshToken: string, email: string, organizationId?: string) => void;
...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  setAuth: (token: string, refreshToken: string, email: string, organizationId?: string) => {
    ...
    if (organizationId) localStorage.setItem("agentepro_org_id", organizationId);
    set({ token, refreshToken, operatorEmail: email, organizationId: organizationId ?? null, isAuthenticated: true });
  },
