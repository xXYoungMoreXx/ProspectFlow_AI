...
function VerifyEmailContent() {
  ...
  const [errorMessage, setErrorMessage] = useState(!token ? 'Token de verificação ausente.' : '');
  ...
  useEffect(() => {
    ...
    const verifyToken = async () => {
      try {
        ...
        setStatus('error');
        setErrorMessage(err.errors?.[0]?.message || 'Falha ao verificar e-mail. O link pode ter expirado.');
      }
    };
      ...
      <CardContent className="pt-6 text-center space-y-4">
        ...
        {status === 'success' && (
          <>
            ...
            <CardDescription>Sua conta foi ativada com sucesso. Você já pode fazer login.</CardDescription>
