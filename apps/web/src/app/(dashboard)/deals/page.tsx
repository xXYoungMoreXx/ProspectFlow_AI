  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Track negotiations and closed contracts</p>
      </div>

      {isLoading ? (
        <div className="space-y-3">
          {[1, 2, 3].map((i) => (
            <Card key={i} className="animate-pulse"><CardContent className="py-4"><div className="h-4 w-40 bg-muted rounded" /></CardContent></Card>
          ))}
        </div>
      ) : deals.length === 0 ? (
        ...
        <div className="space-y-2">
          {deals.map((deal: any) => (
            <Card key={deal.id} className="group hover:border-primary/30 hover:shadow-sm transition-all cursor-pointer">
              <CardContent className="py-3 px-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <Badge variant="outline" className={`text-[10px] ${statusColors[deal.status] || ''}`}>
                      {deal.status}
                    </Badge>
