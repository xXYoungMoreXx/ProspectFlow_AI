...
describe("Multi-Tenant Isolation", () => {
  ...
  const LEAD_B = { id: "lead-b-001", organizationId: ORG_B, contactName: "Beta Lead" };
  ...
  it("org A JWT does not expose org B lead in response", async () => {
    const tokenA = await signTestJWT({ sub: "user-a", organizationId: ORG_A, role: "owner" });
    ...
    if (response.statusCode === 200) {
      const ids = (response.json() as { data: Array<{ id: string }> }).data.map((l) => l.id);
