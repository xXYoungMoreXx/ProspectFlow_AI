...
describe('Security Tests (Phase 6.3)', () => {
  ...
  describe('JWT Security', () => {
    ...
    it('should reject tokens with "none" algorithm', async () => {
      ...
      const payload = Buffer.from(JSON.stringify({ sub: 'op-123', iss: 'agentepro.local', aud: 'agentepro-api', exp: Math.floor(Date.now() / 1000) + 3600 })).toString('base64url');
    ...
    it('should reject tokens with invalid signature', async () => {
      ...
      const differentKeyStr = privateKey.export({ type: 'pkcs8', format: 'pem' }).toString();
  ...
  describe('Injection Prevention', () => {
    it('should block NoSQL/SQLi patterns in agent inputs via validation', async () => {
      mockAgentRepo.findMany.mockResolvedValueOnce({ agents: [], total: 0, nextCursor: null });
  ...
  describe('File Upload Security', () => {
    ...
    it('should reject executable files masquerading as images', async () => {
      ...
      const body = response.json();
      expect(body.errors[0].message).toMatch(/Invalid or blocked file type|Executable files are not allowed/i);
    });
