...
} from "../../../../src/infrastructure/deploy/DeploymentRouter.js";
...
describe("DeploymentRouter", () => {
  it("should use Vercel when quota is sufficient (>= 5)", async () => {
    const mockVercel = {
      ...
      deploy: vi
        .fn()
        .mockResolvedValue(
          ok({ url: "vercel.com", provider: "vercel", deploymentId: "1" }),
        ),
    ...
    const mockNetlify = {
      ...
      deploy: vi
        .fn()
        .mockResolvedValue(
          ok({ url: "netlify.com", provider: "netlify", deploymentId: "2" }),
        ),
    } as unknown as DeploymentProvider;
  ...
  it("should fallback to Netlify if Vercel deploy fails despite having quota", async () => {
    ...
    mockVercel.getRemainingQuota = vi
      .fn()
