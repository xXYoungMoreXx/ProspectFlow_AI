...
export async function authRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/register', {
    ...
    const handler = new RegisterHandler(app.container.db, app.container.authEmailService);
    const result = await handler.execute(parsed.data);
    
    if (result.isErr()) {
      return reply.status(500).send({
        errors: [{ code: 'INTERNAL_ERROR', message: 'Erro ao registrar usuário', requestId: request.requestId }],
      });
    }
  ...
  app.post('/verify-email', {
    ...
    if (result.isErr()) {
      return reply.status(400).send({
        errors: [{ code: 'INVALID_TOKEN', message: 'Token inválido ou expirado', requestId: request.requestId }],
      });
    }
  ...
  app.post('/resend-verification', {
    ...
    const handler = new ResendVerificationHandler(app.container.db, app.container.authEmailService);
  ...
  app.post('/forgot-password', {
    ...
    const handler = new ForgotPasswordHandler(app.container.db, app.container.authEmailService);
    await handler.execute(parsed.data.email);
  ...
  app.post('/reset-password', {
    ...
    const result = await handler.execute(parsed.data.token, parsed.data.password);
  ...
  app.post('/login', {
    ...
    if (result.isErr()) {
      ...
      return reply.status(401).send({
        errors: [{ code: 'AUTHENTICATION_ERROR', message: result.error.message || 'Credenciais inválidas', requestId: request.requestId }],
      });
    }
