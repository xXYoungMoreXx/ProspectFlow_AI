...
export async function agentRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/', async (request, reply) => {
    ...
    if (!parsed.success) {
      return reply.status(400).send({
        errors: parsed.error.issues.map((i) => ({
          code: 'VALIDATION_ERROR', message: i.message, field: i.path.join('.'), requestId: request.requestId,
        })),
      });
    }
    ...
    const result = await getListHandler.execute({
      ...
      status: parsed.data.status as 'ACTIVE' | 'INACTIVE' | 'PAUSED' | undefined,
      persona: parsed.data.persona as 'HUNTER' | 'CLOSER' | 'BUILDER' | 'QA' | undefined,
  ...
  app.post('/', async (request, reply) => {
    ...
    if (result.isErr()) {
      return reply.status(400).send({
        errors: [{ code: 'VALIDATION_ERROR', message: result.error.message, requestId: request.requestId }],
      });
    }
  ...
  app.get<{ Params: { id: string } }>('/:id', async (request, reply) => {
    ...
    if (result.isErr()) {
      return reply.status(404).send({
        errors: [{ code: 'NOT_FOUND', message: result.error.message, requestId: request.requestId }],
      });
    }
  ...
  app.patch<{ Params: { id: string } }>('/:id', async (request, reply) => {
    ...
    const result = await updateHandler.execute(request.params.id, request.operatorId, parsed.data);
    if (result.isErr()) {
      const status = result.error.message.includes('not found') ? 404 : 400;
      return reply.status(status).send({
        errors: [{ code: status === 404 ? 'NOT_FOUND' : 'VALIDATION_ERROR', message: result.error.message, requestId: request.requestId }],
      });
    }
  ...
  app.post<{ Params: { id: string } }>('/:id/activate', async (request, reply) => {
    ...
    const result = await activateHandler.execute(request.params.id, request.operatorId);
    if (result.isErr()) {
      const status = result.error.message.includes('not found') ? 404 : 400;
      return reply.status(status).send({
        errors: [{ code: status === 404 ? 'NOT_FOUND' : 'VALIDATION_ERROR', message: result.error.message, requestId: request.requestId }],
  ...
  app.post<{ Params: { id: string } }>('/:id/pause', async (request, reply) => {
    ...
    const result = await pauseHandler.execute(request.params.id, request.operatorId, body?.['reason'] as string | undefined);
    if (result.isErr()) {
      const status = result.error.message.includes('not found') ? 404 : 400;
      return reply.status(status).send({
        errors: [{ code: status === 404 ? 'NOT_FOUND' : 'VALIDATION_ERROR', message: result.error.message, requestId: request.requestId }],
  ...
  app.post<{ Params: { id: string } }>('/:id/skills', async (request, reply) => {
    ...
    const result = await handler.execute(request.params.id, request.operatorId, parsed.data);
    if (result.isErr()) {
      const status = result.error.message.includes('not found') ? 404 : 400;
      return reply.status(status).send({
        errors: [{ code: status === 404 ? 'NOT_FOUND' : 'VALIDATION_ERROR', message: result.error.message, requestId: request.requestId }],
  ...
  app.patch<{ Params: { id: string; skillId: string } }>('/:id/skills/:skillId', async (request, reply) => {
    ...
    const result = await handler.execute(request.params.id, request.params.skillId, request.operatorId, parsed.data);
    if (result.isErr()) {
      const status = result.error.message.includes('not found') ? 404 : 400;
      return reply.status(status).send({
        errors: [{ code: status === 404 ? 'NOT_FOUND' : 'VALIDATION_ERROR', message: result.error.message, requestId: request.requestId }],
  ...
  app.delete<{ Params: { id: string; skillId: string } }>('/:id/skills/:skillId', async (request, reply) => {
    const handler = new RemoveSkillHandler(app.container.agentRepo);
    const result = await handler.execute(request.params.id, request.params.skillId, request.operatorId);
    if (result.isErr()) {
      const status = result.error.message.includes('not found') ? 404 : 400;
      return reply.status(status).send({
        errors: [{ code: status === 404 ? 'NOT_FOUND' : 'VALIDATION_ERROR', message: result.error.message, requestId: request.requestId }],
  ...
  app.patch<{ Params: { id: string; ruleId: string } }>('/:id/rules/:ruleId', async (request, reply) => {
    ...
    const result = await handler.execute(request.params.id, request.params.ruleId, request.operatorId, parsed.data);
    if (result.isErr()) {
      const status = result.error.message.includes('not found') ? 404 : 400;
      return reply.status(status).send({
        errors: [{ code: status === 404 ? 'NOT_FOUND' : 'VALIDATION_ERROR', message: result.error.message, requestId: request.requestId }],
  ...
  app.delete<{ Params: { id: string; ruleId: string } }>('/:id/rules/:ruleId', async (request, reply) => {
    const handler = new RemoveRuleHandler(app.container.agentRepo);
    const result = await handler.execute(request.params.id, request.params.ruleId, request.operatorId);
    if (result.isErr()) {
      const status = result.error.message.includes('not found') ? 404 : 400;
      return reply.status(status).send({
        errors: [{ code: status === 404 ? 'NOT_FOUND' : 'VALIDATION_ERROR', message: result.error.message, requestId: request.requestId }],
