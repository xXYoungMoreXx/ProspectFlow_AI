...
export async function publicDealRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get<{ Params: { id: string }, Querystring: { token: string } }>('/:id/proposal', async (request, reply) => {
    ...
    return reply.status(200).send({
      message: "Please submit a POST to /:id/accept with the token and contractText to accept.",
      dealId: request.params.id,
      token: request.query.token
    });
  });

  app.post<{ Params: { id: string }, Querystring: { token: string }, Body: z.infer<typeof AcceptProposalSchema> }>('/:id/accept', async (request, reply) => {
    const parsed = AcceptProposalSchema.safeParse(request.body);
    if (!parsed.success) {
      return reply.status(400).send({
        errors: parsed.error.issues.map((i) => ({ code: 'VALIDATION_ERROR', message: i.message, field: i.path.join('.') })),
      });
    }

    if (!request.query.token) {
      return reply.status(401).send({ errors: [{ code: 'UNAUTHORIZED', message: 'Token is required' }] });
    }
    ...
    if (result.isErr()) {
      return reply.status(400).send({ errors: [{ code: 'VALIDATION_ERROR', message: result.error.message }] });
    }
