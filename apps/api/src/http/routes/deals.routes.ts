...
import { GetDealsHandler, GetDealByIdHandler, CancelDealHandler } from '../../application/deal/deal.handlers.js';
...
export async function dealRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post<{ Params: { id: string } }>('/:id/cancel', async (request, reply) => {
    ...
    const result = await cancelHandler.execute(request.params.id, request.operatorId, parsed.data.reason);
    if (result.isErr()) {
  ...
  app.post<{ Params: { id: string } }>('/:id/quote', async (request, reply) => {
    const handler = new GenerateQuoteHandler(app.container.dealRepo, app.container.leadRepo, app.container.llm);
    ...
    if (result.isErr()) {
      const status = result.error.message.includes('not found') ? 404 : 400;
      return reply.status(status).send({ errors: [{ code: status === 404 ? 'NOT_FOUND' : 'GENERATION_ERROR', message: result.error.message, requestId: request.requestId }] });
    }
