...
export class HITLExpirationWorker {
  ...
  start() {
    this.queueAdapter.createWorker('hitl-expiration', async (job: Job<{ approvalId: string }>) => {
      ...
      const [row] = await this.db.select().from(schema.hitlApprovals).where(eq(schema.hitlApprovals.id, approvalId)).limit(1);

      if (!row || row.status !== 'PENDING') {
        console.debug(`[HITLExpirationWorker] Approval ${approvalId} not found or not pending. Skipping.`);
        return;
      }
      ...
      const approval = await this.hitlRepository.findById(approvalId, row.operatorId);
      
      if (!approval) return;

      if (approval.isFinancial) {
        console.info(`[HITLExpirationWorker] Approval ${approvalId} is HITL-FINANCEIRO. Never auto-expires. Dispatching Telegram alert.`);
        // Emit alert event or handle Telegram logic
        return;
      }
      ...
      if (approval.canAutoApprove) {
        ...
        if (result.isOk()) {
          ...
          console.info(`[HITLExpirationWorker] HITL approval ${approvalId} (HITL-2) automatically APPROVED.`);
        }
        return;
      }
      ...
      approval.expire();
      await this.hitlRepository.save(approval);
      console.info(`[HITLExpirationWorker] HITL approval ${approvalId} automatically EXPIRED.`);
