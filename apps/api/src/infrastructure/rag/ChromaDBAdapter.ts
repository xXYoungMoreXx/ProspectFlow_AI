...
export class ChromaDBAdapter {
  ...
  async getOrCreateCollection(name: string): Promise<string> {
    ...
    if (!createRes.ok) {
      ...
      throw new Error(`ChromaDB Error creating collection [${createRes.status}]: ${err}`);
    }
   ...
   */
  async upsertDocuments(collectionName: string, docs: DocumentPayload[]): Promise<void> {
    ...
    const res = await fetch(`${this.baseUrl}/collections/${collectionId}/upsert`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ids,
    ...
    if (!res.ok) {
      ...
      throw new Error(`ChromaDB Error upserting documents [${res.status}]: ${err}`);
    }
  }
   ...
   */
  async query(collectionName: string, queryText: string, nResults: number = 5): Promise<QueryResult[]> {
    const collectionId = await this.getOrCreateCollection(collectionName);

    const res = await fetch(`${this.baseUrl}/collections/${collectionId}/query`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    if (!res.ok) {
      const err = await res.text();
      throw new Error(`ChromaDB Error querying documents [${res.status}]: ${err}`);
    }
  ...
  async deleteCollection(name: string): Promise<void> {
    ...
    if (!res.ok && res.status !== 404) {
      const err = await res.text();
      throw new Error(`ChromaDB Error deleting collection [${res.status}]: ${err}`);
    }
  }
}
