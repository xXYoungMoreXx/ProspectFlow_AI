...
export const messageChannelEnum = pgEnum('message_channel', ['WHATSAPP', 'EMAIL', 'INTERNAL']);
export const dealStatusEnum = pgEnum('deal_status', ['PROPOSED', 'NEGOTIATING', 'CLOSED', 'CANCELLED']);
...
export const agents = pgTable('agents', {
  ...
  llmTemperature: numeric('llm_temperature', { precision: 3, scale: 2 }).notNull().default('0.70'),
  llmMaxTokens: integer('llm_max_tokens').notNull().default(4096),
  llmSystemPrompt: text('llm_system_prompt'),
  tokenBudgetTotal: bigint('token_budget_total', { mode: 'number' }).notNull().default(1_000_000),
  tokenBudgetRemaining: bigint('token_budget_remaining', { mode: 'number' }).notNull().default(1_000_000),
  ragEnabled: boolean('rag_enabled').notNull().default(false),
  ragCollection: text('rag_collection'),
  ragTopK: integer('rag_top_k').default(5),
  ragThreshold: numeric('rag_threshold', { precision: 3, scale: 2 }).default('0.70'),
...
export const messages = pgTable('messages', {
  ...
  index('idx_messages_lead').on(table.leadId, table.createdAt),
]);
...
export const deals = pgTable('deals', {
  ...
  basePriceCents: bigint('base_price_cents', { mode: 'number' }).notNull().default(0),
...
export const projects = pgTable('projects', {
  ...
  index('idx_projects_status').on(table.status, table.operatorId),
]);
