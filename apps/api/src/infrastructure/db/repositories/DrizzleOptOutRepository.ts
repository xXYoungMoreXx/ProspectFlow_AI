...
export class DrizzleOptOutRepository implements OptOutRepository {
  ...
  async save(optOut: OptOut): Promise<void> {
    ...
    await this.db.insert(schema.prospectOptouts).values({
      id: json.id,
  ...
  async isBlocked(operatorId: string, phoneHash?: string, emailHash?: string): Promise<boolean> {
    if (!phoneHash && !emailHash) return false;

    const conditions = [];
    if (phoneHash) conditions.push(eq(schema.prospectOptouts.phoneHash, phoneHash));
    if (emailHash) conditions.push(eq(schema.prospectOptouts.emailHash, emailHash));
