 ...
 */
import type { AgentRepository, CreateRuleData, UpdateRuleData } from '../../domain/agent/AgentRepository.js';
...
export class UpdateRuleHandler {
  ...
  async execute(agentId: string, ruleId: string, operatorId: string, updates: UpdateRuleData): Promise<Result<AgentRule, Error>> {
    const agent = await this.repo.findById(agentId, operatorId);
    if (!agent) return err(new NotFoundError('Agent', agentId));
...
export class RemoveRuleHandler {
  ...
  async execute(agentId: string, ruleId: string, operatorId: string): Promise<Result<void, Error>> {
    const agent = await this.repo.findById(agentId, operatorId);
    if (!agent) return err(new NotFoundError('Agent', agentId));
...
export class ListRulesHandler {
  ...
  async execute(agentId: string, operatorId: string): Promise<Result<AgentRule[], Error>> {
    const agent = await this.repo.findById(agentId, operatorId);
    if (!agent) return err(new NotFoundError('Agent', agentId));
