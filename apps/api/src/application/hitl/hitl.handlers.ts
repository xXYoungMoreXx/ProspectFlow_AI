...
export class ApproveHITLHandler {
  ...
  async execute(approvalId: string, operatorId: string, note?: string): Promise<Result<HITLApproval, Error>> {
    ...
    try {
      ...
      console.info(`[ApproveHITLHandler] Notified Agent Runtime of approval for ${approvalId}`);
    } catch (error) {
      console.error(`[ApproveHITLHandler] Failed to notify Agent Runtime:`, error);
