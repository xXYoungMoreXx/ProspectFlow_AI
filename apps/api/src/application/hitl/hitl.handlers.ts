...
export class GetPendingApprovalsHandler {
  ...
  async execute(operatorId: string, organizationId: string = "org_mvp"): Promise<HITLApproval[]> {
...
export class ApproveHITLHandler {
  ...
  async execute(
    ...
    const approval = await this.repo.findById(approvalId, operatorId, organizationId);
  ...
  private async triggerDeploy(
    ...
    const project = await this.projectRepo!.findById(projectId, operatorId, "org_mvp");
    if (!project) {
      console.warn(
