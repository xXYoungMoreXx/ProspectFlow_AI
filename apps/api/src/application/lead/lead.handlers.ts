...
export class UpdateLeadStatusHandler {
  ...
  async execute(leadId: string, operatorId: string, status: LeadStatus, reason?: string): Promise<Result<Lead, Error>> {
...
export class GetLeadByIdHandler {
  ...
  async execute(leadId: string, operatorId: string): Promise<Result<Lead, NotFoundError>> {
    const lead = await this.repo.findById(leadId, operatorId);
    if (!lead) return err(new NotFoundError('Lead', leadId));
    return ok(lead);
  }
