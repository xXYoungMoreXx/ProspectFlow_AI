...
export class GenerateProposalLinkHandler {
  ...
  async execute(input: {
    ...
    if (!input.contractHash) return err(new ValidationError('contractHash is required'));
    ...
    try {
      ...
      return err(new ValidationError(`Failed to generate proposal link: ${(e as Error).message}`));
    }
  }
}
