...
export class ContractAcceptance {
  ...
  static recordAcceptance(input: {
    ...
    if (!input.contractText) return err(new ValidationError('contractText is required'));

    const contractHash = createHash('sha256').update(input.contractText).digest('hex');
    const ipHash = createHash('sha256').update(input.ipRaw).digest('hex');
    const userAgentHash = createHash('sha256').update(input.userAgent).digest('hex');

    const now = new Date();

    return ok(new ContractAcceptance({
      id: input.id,
      dealId: input.dealId,
      contractHash,
      ipHash,
  ...
  get contractHash(): string { return this.props.contractHash; }
  get acceptedAt(): Date { return this.props.acceptedAt; }
  get ipHash(): string { return this.props.ipHash; }
  get userAgentHash(): string { return this.props.userAgentHash; }
