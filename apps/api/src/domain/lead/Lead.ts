...
export class Lead extends AggregateRoot {
  ...
  static create(input: {
    ...
    if (input.contact.name.length < 1 || input.contact.name.length > 200) {
      return err(new ValidationError('Contact name must be between 1 and 200 chars', 'contact_name'));
    }
  ...
  get contact(): ContactInfo { return this.props.contact; }
  get source(): LeadSource { return this.props.source; }
  get qualificationScore(): number | undefined { return this.props.qualificationScore; }
  ...
  qualify(score: number, agentId: string): Result<void, ValidationError> {
    if (score < 0 || score > 100) {
      return err(new ValidationError('Score must be between 0 and 100', 'qualification_score'));
    }
