...
export class OptOut {
  ...
  static addToBlocklist(input: {
    ...
    if (!input.phoneRaw && !input.emailRaw) {
      return err(new ValidationError('At least one of phone or email must be provided to opt out'));
    }

    const phoneHash = input.phoneRaw ? createHash('sha256').update(input.phoneRaw).digest('hex') : null;
    const emailHash = input.emailRaw ? createHash('sha256').update(input.emailRaw).digest('hex') : null;

    return ok(new OptOut({
      id: input.id,
  ...
  get phoneHash(): string | null { return this.props.phoneHash; }
  get emailHash(): string | null { return this.props.emailHash; }
  get optedOutAt(): Date { return this.props.optedOutAt; }
