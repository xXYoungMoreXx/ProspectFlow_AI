...
export class Project extends AggregateRoot {
  ...
  get deliverableUrl(): string | undefined { return this.props.deliverableUrl; }
  get lighthouse(): LighthouseScores { return this.props.lighthouse; }
  get revisionCount(): number { return this.props.revisionCount; }
  ...
  requestRevision(notes: string): Result<void, ValidationError> {
    if (this.props.status === 'CANCELLED') {
      return err(new ValidationError('Cannot request revision on cancelled project'));
    }
    ...
    this.props.updatedAt = new Date();
    this.addDomainEvent(
      createDomainEvent('project.revision_requested', 'Project', this.props.id, {
        projectId: this.props.id,
        notes,
