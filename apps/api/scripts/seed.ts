...
async function seed() {
  ...
  // 1. Setup connection
  const dbUrl = process.env.DATABASE_URL || 'postgres://agentepro:agentepro_dev@localhost:5432/agentepro';
  ...
  try {
    ...
    const [admin] = await db.insert(schema.operators).values({
    ...
    const [hunterAgent] = await db.insert(schema.agents).values({
      operatorId: admin.id,
      name: 'Hunter Pro',
      persona: 'HUNTER',
      status: 'ACTIVE',
    ...
    const [closerAgent] = await db.insert(schema.agents).values({
      operatorId: admin.id,
      name: 'Closer Elite',
      persona: 'CLOSER',
      status: 'ACTIVE',
    ...
    const [builderAgent] = await db.insert(schema.agents).values({
      operatorId: admin.id,
      name: 'Builder AI',
      persona: 'BUILDER',
      status: 'ACTIVE',
