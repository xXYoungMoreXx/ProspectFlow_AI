...
/**
 * Integration test config — requires Docker running (Testcontainers spins up PostgreSQL).
 *
 * Run: npm run test:integration
 */
export default defineConfig({
  resolve: {
    // Ensure type augmentations in src/types are visible to test files
    conditions: ["node"],
  },
  test: {
    environment: "node",
    ...
    hookTimeout: 120_000,
    testTimeout: 60_000,
    // Serial execution prevents concurrent DB access issues
    pool: "forks",
