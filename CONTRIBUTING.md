...
**É TERMINANTEMENTE PROIBIDO fazer _git push_ direto para a branch `main`.**
...
2. **Pull Requests (PRs)**: Se você deseja que sua atualização chegue à `main`, você **deve abrir um Pull Request de `develop` direcionado para a branch `main`** (fluxo: feature branch → `develop` → PR → `main`).
...
4. **Validação**: A sua PR deve obrigatoriamente passar por todos os _checks_ do GitHub Actions.
5. **Revisão Manual**: Se os _checks_ passarem, a PR será avaliada **manualmente** por mim. Eu irei aprovar ou rejeitar a PR. Em caso de rejeição, deixarei uma justificativa clara do motivo.
